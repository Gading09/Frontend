...
sudo docker build -f "Dockerfile" -t gading09/deployment:fix-$TRAVIS_BUILD_NUMBER  --label "Maintaner  Hedy <hedy@alterra.id>" .
# push apps image to docker hub
sudo docker push gading09/deployment:fix-$TRAVIS_BUILD_NUMBER
...
kubectl -n alta10 --kubeconfig kubeconfig set image deployment/${APP_K8S_DEPLOYMENT_NAME} ${APP_K8S_CONTAINER_NAME}=gading09/deployment:fix-${TRAVIS_BUILD_NUMBER}